...
| 파일 | 설명 |
|------|------|
| `null.merged.gene.SummarizedExperiment.rds` | nf-core/rnaseq 파이프라인이 생성한 SummarizedExperiment 객체. Salmon 정량 결과(gene-level)를 포함 |

> **참고:** Count matrix는 `salmon.merged.gene_counts_length_scaled` assay를 사용합니다.  
> 이는 라이브러리 크기와 평균 전사체 길이를 모두 보정한 값입니다.
...
nf-core/rnaseq 결과(.rds)를 로드하여 DESeq2 차등 발현 분석을 수행합니다.
...
- SummarizedExperiment의 `rowData`에서 `gene_name`을 추출하여 DESeq2 객체에 매핑
...
#    DESeq2, SummarizedExperiment 등은 Bioconductor에서 설치
install.packages("BiocManager")
BiocManager::install(c("DESeq2", "SummarizedExperiment", "fgsea"))
...
| `SummarizedExperiment` | Bioconductor | 입력 데이터 구조 |
